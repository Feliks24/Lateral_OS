...
\noindent
Der Kernel ist der Kern des Betriebssystems und stellt die grundlegende Infrastruktur bereit, die
...
\noindent
Bei dem Makrokernarchitektur werden alle Services innerhalb des Kernels implementiert. Dies umfasst Scheduling, Dateisystem, Networking, Gerätetreiber, Speichermanagement und Paging.

\paragraph{Vorteile}
\begin{itemize}
   \item Packtet werden vom Kernel direkt an den Adressraum des Empfängers gesendet. Dadurch müssen Pakete nicht wiederholt Kopiert werden. 
   \item Die Eingabeverarbeitung (Input Processing) kann auf der Interrupt-Ebene erfolgen, was die Latenz reduziert. Um die Latenz auf ein Minimum zu reduzieren, können Teile des Protokolls im Interrupt-Handler des Kernels platziert werden, wodurch ein zusätzlicher Dispatch und RPC entfallen.
\end{itemize}

\paragraph{Nachteile}
\begin{itemize}
   \item Die TCB ist groß, da fast die gesamte Systemfunktionalität, einschließlich aller Treiber und Dienste, innerhalb des Kernels als absolut vertrauenswürdig implementiert werden muss
...
\noindent
Die Mikrokernalarchitektur basiert auf der Idee den Kernel zu minimieren, deshalb werden alle höheren Services auf die Nutzterebene verlagert. Der Kernel bietet nur essentielle Mechanismen wie Adressräume, Interprozesskommunikation (IPC) und grundlegendes Scheduling,
\paragraph{Vorteile}
\begin{itemize}
...
\hline
\textbf{Aspekt} & \textbf{Makrokernel} & \textbf{Mikrokernel (1. Gen., Mach 3.0 / UX)} & \textbf{Mikrokernel (2. Gen., L4 / optimiertes Mach 3.0)} \\ \hline
UDP RTT (Mach) & 4,8 ms & 19,5 ms (unoptimiert) & 4,2 ms (neuer, optimierter Server) \\ \hline
Kosten IPC/RPC (486-DX50) & Unix Syscall: ca. 20 µs & Mach IPC: ca. 115 µs & L4 IPC: ca. 5 µs \\ \hline
Ursache Ineffizienz & Gering & Schlechte Implementierungsstruktur (altes Kernel-Design im User-Space) & — \\ \hline
\end{tabular}
}
\caption{Vergleich zwischen Makrokernel und Mikrokernel-Architekturen.}
\label{tab:kernel-vergleich}
\end{table}
