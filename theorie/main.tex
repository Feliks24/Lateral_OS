...
\item Wobei später gdb wieder ging aber nicht ausgereicht hat um Sachen ausreichend zu fixen
\item[]
\item Memory remap schien noch ganz ok zu gehen
\item Interrupt Vektor Table und Stack initieren war etwas schwierig, wo
	ultimativ die Base Funktion nicht funktioniert hat und somit wir die
	erweiterte Funktionalität nicht geschaft haben
\item[]
\item Code kompeliert
\item Zuvor ging der test print durch nach dem init stack aber es hat keine interrupts behandelt
\item Jetzt hat es Probleme nach dem stack init und hört danach auf
\end{itemize}
