...
\section*{Aufgabe \sheetNR-2(Behandlung von Ausnahmen)}
\begin{itemize}
\item Wir haben leider nicht geschafft den Code rechtzeitig zum Laufen zu bringen
\item Es war einfach zu wenig Hilfestellung und kein guter Weg zu testen das es läuft
\item Gdb schien uns auch nicht helfen zu wollen
\tem[]
\item Memory remap schien noch ganz ok zu gehen
\item Interrupt Vektor Table zu schreiben und Stack zu initieren haben wir die Idee von verstanden aber selbst der Grundlegende Code hat nicht geklappt ohne das wir überhaupt wissen was passiert
\item Hat es dann ultimativ sehr frustrierend gemacht weil man keine Ahnung hat was schief läuft oder was gemacht werden soll
\item[]
\item Der Code kompeliert und scheint durchzulaufen so das es an ein Print statement kommt aber es kommt nicht dazu, dass der Interrupt bemerkt wird
\end{itemize}
